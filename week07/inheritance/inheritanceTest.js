...
(() => {
  ...
  class Person {
    // refactoring Martin Fowler: replace inheritance with delegation
    constructor(name) {
      this.name = name;
      this.worklog = [];
    }
    mustDo() {
      // design pattern: template method (abstract method)
      return "";
    }
    work() {
...
(() => {
  ...
  const dk = Person({ work: () => "" }); // configure object to have a reference to Person.prototype
